...
def get_data_rub(execution_date):
    ...
    url = f'https://www.cbr-xml-daily.ru/archive/{year}/{int(month):02d}/{int(day):02d}/daily_json.js'
    sleep(10)
    response = requests.get(url, headers=headers)
    currency_data = response.json()

    currencies_date = datetime.strptime(currency_data['Date'][:10], '%Y-%m-%d')
    currencies = [currencies[1] for currencies in currency_data["Valute"].items()]

    currencies_list = [{**currency, 'date': currencies_date} for currency in currencies]
    rows = [list(currency.values()) for currency in currencies_list]
    fields = list(currencies_list[0].keys())
...
with DAG('currency_parsing',
    ...
    get_data_pound >> fetch_data_pound
    get_data_usd >> fetch_data_usd
    get_data_rub >> fetch_data_rub
    get_data_kzt >> fetch_data_kzt
